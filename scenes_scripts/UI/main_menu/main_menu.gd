...
# Remembers if the game just launched so we don't play the fade twice
static var has_played_splash: bool = false
...
func _ready() -> void:
  ...
  # Check if we should play the splash animation
  if not has_played_splash:
    has_played_splash = true
    # 1. Load the main menu in the background
    _show_menu(MenuType.MAIN_MENU)
    # 2. Trigger the splash screen curtain to do its fade
    splash_screen.play_fade_animation()
  else:
    # If returning to menu from a game, just hide the splash instantly
