...
func process_voice(buffer: PackedByteArray):
	get_sample_rate()

	var decompressed_voice: Dictionary = Steam.decompressVoice(buffer, current_sample_rate)
...
func send_voice(buffer: PackedByteArray):
